...
# setup include paths
PACKAGE_C_INCLUDES += -I $(QNN_SDK_ROOT)/include -I $(QNN_SDK_ROOT)/include/GPU -I $(LOCAL_PATH)/../include/ -I $(UTIL_SRC_DIR) -I $(UTIL_SRC_DIR)/GPU -I $(CUSTOM_OP_DIR)

# copy source files from SDK if not present
$(info Copying custom op source files from SDK)
COPYFILES := $(shell find $(CUSTOM_OP_DIR)/GPU -name "*.cpp" -exec cp -rf {} $(LOCAL_PATH)/../src 2>/dev/null \;)
...
LOCAL_MODULE                   := HardSigmodUdoPackage
