                                           ...
                                           std::unique_ptr<SnpeUdo_TensorInfo_t>& tensorInfo)
{
    ...
    else
    {
        ...
        tensorInfo->tensorName = new char[tensorName.size()+1];
        std::copy(tensorName.begin(), tensorName.end(), tensorInfo->tensorName);
        tensorInfo->tensorName[tensorName.size()] = '\0';
...
UdoRegLibrary::registerValidationFunction(const std::string &name,
    ...
    UDO_VALIDATE_MSG(pos != m_ValidateFunctions.end(),
                 SNPE_UDO_INVALID_ARGUMENT,
                 "Validation for op: " << name << " with core-type: "<<
                 static_cast<typename std::underlying_type<SnpeUdo_CoreType_t>::type>(coreType)<<
